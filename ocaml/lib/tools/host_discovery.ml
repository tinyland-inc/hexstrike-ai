...
let execute (args : Yojson.Safe.t) : (string, string) result =
  ...
  if target = "" then Error "target is required"
    ...
    | Ok res ->
      if res.exit_code = 0 then Ok (Tool_output.wrap_json ~tool_name:"host_discovery" ~target res)
      else Ok (Tool_output.wrap_error ~tool_name:"host_discovery" ~target res)
    | Error e -> Error e
